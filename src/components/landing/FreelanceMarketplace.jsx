...
function MockJobCard({ delay }) {
  return (
    <motion.div {...fadeUp(delay)} className="rounded-xl border border-white/[0.08] bg-[#0a0f1e]/80 p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold text-green-400 bg-green-400/10 border border-green-400/20">Open</span>
      ...
      <div className="flex items-center gap-2 text-[10px] text-slate-400 mb-2">
        <DollarSign size={10} className="text-green-400" /> $800 - $1,200 (fixed)
      </div>
...
function MockFreelancerCard({ delay }) {
  return (
    <motion.div {...fadeUp(delay)} className="rounded-xl border border-white/[0.08] bg-[#0a0f1e]/80 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="ml-auto px-1.5 py-0.5 rounded-full text-[9px] font-semibold text-green-400 bg-green-400/10 border border-green-400/20">Available</span>
      </div>
      ...
      <div className="flex items-center gap-3 text-[10px] text-slate-400">
        ...
        <span className="flex items-center gap-1"><Star size={9} className="text-yellow-400" /> 12 jobs</span>
      </div>
    </motion.div>
  );
}
...
function MockMilestone() {
  return (
    <motion.div {...fadeUp(0.3)} className="rounded-xl border border-white/[0.08] bg-[#0a0f1e]/80 p-4">
      <h4 className="text-[10px] font-bold text-blue-300/70 uppercase tracking-wider mb-3">Milestone Progress</h4>
      {[
...
const FEATURES = [
  { icon: Shield, title: 'Milestone-based Delivery', desc: 'Pecahan pekerjaan jadi milestone, submit dan approve bertahap' },
  { icon: Zap, title: 'Auto Workspace', desc: 'Kolaborasi otomatis dengan Kanban, Chat, dan File Storage' },
  { icon: Users, title: 'Verified Profiles', desc: 'Portofolio terverifikasi dan rating dari klien sebelumnya' },
  { icon: MessageSquare, title: 'Integrated Chat', desc: 'Komunikasi langsung dalam workspace tanpa keluar platform' },
];
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <motion.p {...fadeUp(0.1)} className="text-slate-400 text-base sm:text-lg leading-relaxed mb-8 max-w-xl">
              Freelance marketplace terintegrasi — dari hiring sampai delivery, semua dalam satu platform. Workspace otomatis, milestone tracking, dan komunitas builder.
            </motion.p>
            ...
            <div className="grid grid-cols-2 gap-3 mb-8">
              {FEATURES.map((f, i) => {
                const Icon = f.icon;
                return (
                  <motion.div key={f.title} {...fadeUp(0.15 + i * 0.05)} className="flex items-start gap-3">
            ...
            <motion.div {...fadeUp(0.4)} className="flex flex-wrap gap-3">
              <Link to="/freelance" className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-blue-500 to-purple-600 hover:opacity-90 transition-all shadow-lg shadow-blue-500/20">
                Cari Freelancer <ArrowRight size={15} />
              </Link>
          ...
          <motion.div {...fadeUp(0.2)} className="relative">
            ...
            <div className="relative grid grid-cols-2 gap-3 p-4">
              <div className="space-y-3">
                <MockJobCard delay={0.15} />
                <MockFreelancerCard delay={0.2} />
              </div>
              <div className="space-y-3 pt-8">
                <MockJobCard delay={0.25} />
                <MockMilestone />
              </div>
            </div>
          </motion.div>
